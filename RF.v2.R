...
vector1 <- vector(mode = "numeric", length = nrOfTimeSteps*nrOfPixels)
vector2 <- vector(mode = "numeric", length = nrOfTimeSteps*nrOfPixels)
vector3 <- vector(mode = "numeric", length = nrOfTimeSteps*nrOfPixels)
vector4 <- vector(mode = "numeric", length = nrOfTimeSteps*nrOfPixels)

#predictor 1
setwd(path1)
dataMatrix <- matrix(0, nrOfTimeSteps, nrOfPixels)
...
for(m in fileList1){
        ...
        for(z in 1:12){
                ...
                tempRaster <- raster(t(tempRaster), xmn=-180, xmx=180, ymn=-90, ymx=90)
                tempRaster <- crop(tempRaster, extent(c(-12, 45, 30, 72)))
...
pred_m <- matrix(RF$predicted, nrOfPixels, nrOfTimeSteps)
st <- stack()
for (i in 1:nrOfTimeSteps){
        layer <- matrix(pred_m[,i], 14, 19)
        rast <- raster(layer)
        st <- addLayer(st, rast)
}
prediction <- st

y_m <- matrix(RF$y, nrOfPixels, nrOfTimeSteps)
st <- stack()
for (i in 1:nrOfTimeSteps){
        layer <- matrix(pred_m[,i], 14, 19)
        rast <- raster(layer)
