...
path <- "//TEC-JULIA-FRA/Users/julia_francesca/Documents/Data/"
setwd(path)
...
wmap <- readOGR(dsn=paste(path,"3_shapefiles",sep=""), layer="ne_110m_land")
grid <- readOGR(dsn=paste(path,"3_shapefiles",sep=""),layer="ne_110m_graticules_30")
...
st <- stack(paste(path,
                  "4_Results/EOF/TP_eof.nc",sep=""),
            bands=c(1:12),varname="TP.eof")
...
#         newRast <- merge(tempRast_22,tempRast_11)
        ...
        st_new <- addLayer(st_new,t(newRast))        
...
brks <- c(-0.09,-0.05,-0.02,-0.01,-0.005,0,0.005,0.01,0.02,0.05, 0.09)

tiff(filename=paste(path,"4_Results/EOF/TP_EOF_modes.tiff",sep=""),
