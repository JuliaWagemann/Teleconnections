...
varSomName <- "T2M.som"
varUnit <- "deg"
varDesc <- "SOM nodes of Temperature at 2m anomalies"

ncdfName <- "som_t2m_seed10.nc"
########################################################################
...
var.som <- som(data=datMat.sc, grid=somgrid(somCols,somRows,"rectangular"), rlen=trainingRate, alpha=c(0.05, 0.01),
...
par(mfrow=c(4,3))
som.node1 <- codes[1,]
image(matrix(som.node1,ncol=geoLon,nrow=geoLat))
som.node2 <- codes[2,]
image(matrix(som.node2,ncol=geoLon,nrow=geoLat))
som.node3 <- codes[3,]
image(matrix(som.node3,ncol=geoLon,nrow=geoLat))
som.node4 <- codes[4,]
image(matrix(som.node4,ncol=geoLon,nrow=geoLat))
som.node5 <- codes[5,]
image(matrix(som.node5,ncol=geoLon,nrow=geoLat))
som.node6 <- codes[6,]
image(matrix(som.node6,ncol=geoLon,nrow=geoLat))
som.node7 <- codes[7,]
image(matrix(som.node7,ncol=geoLon,nrow=geoLat))
som.node8 <- codes[8,]
image(matrix(som.node8,ncol=geoLon,nrow=geoLat))
som.node9 <- codes[9,]
image(matrix(som.node9,ncol=geoLon,nrow=geoLat))
som.node10 <- codes[10,]
image(matrix(som.node10,ncol=geoLon,nrow=geoLat))
som.node11 <- codes[11,]
image(matrix(som.node11,ncol=geoLon,nrow=geoLat))
som.node12 <- codes[12,]
image(matrix(som.node12,ncol=geoLon,nrow=geoLat))
...
netCDF_varDef <- ncvar_def(varSomName, units=varUnit,dim=list(x,y,soms), missval=NA,
